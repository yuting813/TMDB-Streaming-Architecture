...
> **安全規則**：`customers/{uid}` 及其子集合（`subscriptions`、`checkout_sessions`、`payments`）均限制為 `request.auth.uid == uid`。`products/**` 因僅含方案元資料，設為公開可讀（`if true`）。訂閱記錄的寫入權限保留給 Stripe Firebase Extension webhook，不允許任何用戶端寫入。
